...
int main (int argc, char** argv) {
	...
	while(1) {
		...
		if (strlen(cmdLine)>1)
			idxx=parseString(cmdLine, &exeName);
		else
			continue;
		...
		if (pid == 0) {
			...
			if(idxx==0){
				if (execle(newPath, argVect[0],environ)==-1) {
					perror("myShell");
					exit(errno*-1);
				}
			}
			if(idxx==1){
				if (execle(newPath, argVect[0],argVect[1],environ)==-1) {
					perror("myShell");
					exit(errno*-1);
				}
			}
			if(idxx==2){
				if (execle(newPath, argVect,argVect[1],argVect[2],environ)==-1) {
					perror("myShell");
					exit(errno*-1);
				}
			}
			if(idxx==3){
				if (execle(newPath, argVect,argVect[1],argVect[2],argVect[3],environ)==-1) {
					perror("myShell");
					exit(errno*-1);
				}
			}
			if(idxx==4){
				if (execle(newPath, argVect,argVect[1],argVect[2],argVect[3],argVect[4],environ)==-1) {
					perror("myShell");
					exit(errno*-1);
				}
			}
			if(idxx==5){
				if (execle(newPath, argVect,argVect[1],argVect[2],argVect[3],argVect[4],argVect[5],environ)==-1) {
					perror("myShell");
					exit(errno*-1);
				}
			}
